...
if sys.version_info < (3,5):
    print('This game requires Python version 3.5 or higher.');
    sys.exit();
...
if pgzero_version < [1,2]:
    print('This game requires a Pygame Zero version 1.2 or higher. \
    You are using version {pgzero.__version__}. You can upgrade \
    with the command: "pip install --upgrade pyzero"')
    sys.exit();

# Screen size of the stage
WIDTH = 800
HEIGHT = 480
TITLE = "BOING! PONG"
...
def normalized(x, y):
    # Return a unit vector for calculating angles the ball should move
    # Get length of vector (x,y) - math.hypot uses Pythagoras' 
    # theorem to get length of hypotenuse
...
class Impact(Actor):
    ...
    def update(self):
        # There are 5 impact sprites numbered 0 to 4. 
        # We update to a new sprite every 2 frames.
        self.image = 'impact' + str(self.time // 2)
        
        # The Game class maintains a list of Impact instances. 
        # In Game.update, if the timer for an object
...
class Ball(Actor):
    def __init__(self, dx):
        ...
        # dx and dy together describe the direction in which the ball is moving. 
        # For example, if (direction x) dx and (direction y) dy are 1 and 0,
        # the ball is moving to the right, with no movement up or down. 
        # If both values are negative, the ball is moving
        # left and up, with the angle depending on the relative values of the two variables. 
        # If you're familiar with vectors, dx and dy represent a unit vector.
    ...
    def update(self):
        ...
        for i in range(self.speed):
            ...
            # To determine whether the ball might collide with a bat, we first measure 
            # the horizontal distance from the ball to the center of the screen, 
            # and check to see if its edge has gone beyond the edge of the bat.
            # The center of each bat is 40 pixels from the edge of the screen, 
            # or to put it another way, 360 pixels from the center of the screen. 
            # The bat is 18 pixels wide and the ball is 14 pixels wide. Given that these
            # sprites are anchored from their centers, when determining if they overlap or touch, 
            # we need to look at their half-widths - 9 and 7. Therefore, if the center of the ball 
            # is 344 pixels from the center of the screen, it can bounce off a bat 
            # (assuming the bat is in the right position on the Y axis - checked shortly afterwards).
            # We also check the previous X position to ensure that this is the first 
            # frame in which the ball crossed the threshold.
            
            if abs(self.x - HALF_WIDTH) >= 344 and abs(original_x - HALF_WIDTH) < 344:
                
                # Now that we know the edge of the ball has crossed the threshold on the x-axis, 
                # we need to check to see if the bat on the relevant side of the arena is 
                # at a suitable position on the y-axis for the
                ...
                if difference_y > -64 and difference_y < 64:
                    ...
                    # To understand the maths used below, we first need to consider what would happen 
                    # with this kind of collision in the real world. The ball is bouncing off a 
                    # perfectly vertical surface. This makes for a pretty simple calculation. 
                    # Let's take a ball which is travelling at 1 meter per second to the right,
                    # and 2 meters per second down. Imagine this is taking place in space, 
                    # so gravity isn't a factor.
                    # After the ball hits the bat, it's still going to be moving at 2 m/s down, 
                    # but it's now going to be moving 1 m/s to the left instead of right. 
                    # So its speed on the y-axis hasn't changed, but its direction on the x-axis has been reversed. 
                    # This is extremely easy to code - "self.dx = -self.dx".
                    ...
                    # Limit the Y component of the vector so we don't get into a 
                    # situation where the ball is bouncing
                    # up and down too rapidly
                    self.dy = min(max(self.dy, -1), 1)

                    # Ensure our direction vector is a unit vector, i.e. represents a 
                    # distance of the equivalent of
                    ...
                    # Add an offset to the AI player's target Y position, 
                    # so it won't aim to hit the ball exactly
            ...
            # The top and bottom of the arena are 220 pixels from the center
            if abs(self.y - HALF_HEIGHT) > 220:
                # Invert vertical direction and apply new dy to y so that the ball 
                # is no longer overlapping with the
                # edge of the arena
                self.dy = -self.dy
                self.y += self.dy
...
class Bat(Actor):
    def __init__(self, player, move_func=None):
        ...
        # move_func is a function we may or may not have been passed by the code 
        # which created this object. If this bat
        # is meant to be player controlled, move_func will be a function that when called, 
        # returns a number indicating
        # the direction and speed in which the bat should move, based on the keys 
        # the player is currently pressing.
        # If move_func is None, this indicates that this bat should instead be controlled 
        # by the AI method.
        ...
        # decide when to create a new ball in the center of the screen - see comments in 
        # Game.update for more on this.
        # Finally, it is used in Game.draw to determine when to display a 
        # visual effect over the top of the background
    ...
    def update(self):
        ...
        # Choose the appropriate sprite. There are 3 sprites per player - 
        # e.g. bat00 is the left-hand player's
        # standard bat sprite, bat01 is the sprite to use when the ball has just bounced 
        # off the bat, and bat02 is the sprite to use when the bat has just missed the 
        # ball and the ball has gone out of bounds.
    ...
    def ai(self):
        ...
        # e.g. 4 means it will move 4 pixels down
        # the screen.

        # To decide where we want to go, we first check to see how far we are from the ball.
        x_distance = abs(game.ball.x - self.x)

        # If the ball is far away, we move towards the center of the screen (HALF_HEIGHT), 
        # on the basis that we don't yet know whether the ball will be in the top 
        # or bottom half of the screen when it reaches our position on
        # the X axis. By waiting at a central position, we're as ready as it's 
        # possible to be for all eventualities.
        target_y_1 = HALF_HEIGHT

        # If the ball is close, we want to move towards its position on the Y axis. 
        # We also apply a small offset which
        # is randomly generated each time the ball bounces. 
        # This is to make the computer player slightly less robotic
        # - a human player wouldn't be able to hit the ball right in the centre 
        # of the bat each time.
        target_y_2 = game.ball.y + game.ai_offset
        
        # The final step is to work out the actual Y position we want to move towards. 
        # We use what's called a weighted average - taking the average of the 
        # two target Y positions we've previously calculated, but shifting the
        # balance towards one or the other depending on how far away the ball is. 
        # If the ball is more than 400 pixels (half the screen width) away on the X axis, 
        # our target will be half the screen height (target_y_1). If the
        # ball is at the same position as us on the X axis, our target will be target_y_2. 
        # If it's 200 pixels away,
        ...
        # then make sure we can't move any further than MAX_AI_SPEED
        # each frame
        return min(MAX_AI_SPEED, max(-MAX_AI_SPEED, target_y - self.y))

class Game:
    def __init__(self, controls=(None, None)):
        # Create a list of two bats, giving each a player number and a function to 
        # use to receive control inputs (or the value None if this is intended 
        # to be an AI player)
    ...
    def update(self):
        ...
        # Remove any expired impact effects from the list. We go through the list backwards, 
        # starting from the last element, and delete any elements those time attribute 
        # has reached 10. We go backwards through the list
        ...
        if self.ball.out():
            ...
            # We use the timer of the player who has just conceded a point to decide when 
            # to create a new ball in the
            # center of the level. This timer starts at zero at the beginning of the game and counts down by one every
            # frame. Therefore, on the frame where the ball first goes off the screen, the timer 
    ...
    def draw(self):
        ...
        # Square brackets are needed around the ball because
        # it's just an object, whereas the other two are lists - 
        # and you can't directly join an object onto a
        ...
        for p in (0,1):
            # Convert score into a string of 2 digits (e.g. "05") 
            # so we can later get the individual digits
            score = '{0:02d}'.format(self.bats[p].score)
            # Inner loop goes through each digit
            for i in (0,1):
                # Digit sprites are numbered 00 to 29, where the first digit is 
                # the color (0 = grey, 1 = blue, 2 = green) and the second digit is 
                # the digit itself
                # Color is usually grey but turns red or green (depending on player number) 
                # when a point has just been scored
                color = '0'
                other_p = 1 - p
    ...
    def play_sound(self, name, count=1, menu_sound=False):
        # Some sounds have multiple varieties. If count > 1, we'll randomly choose one 
        # from those. We don't play any in-game sound effects if player 0 is an AI player
        # as this means we're on the menu
    
        # Some Pygame installations have issues playing ogg sound files. 
        # play_sound can skip sound
...
def update():
    ...
    # Work out whether the space key has just been pressed - i.e. 
    # in the previous frame it wasn't down,
...
# Create a new Game object, without any players
game = Game()
# Tell Pygame Zero to start - this line is only required when running the game from an IDE such 
# as IDLE or PyCharm
pgzrun.go()
